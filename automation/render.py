...
def render_template(j2_file: str, vars_file: str, file=None):
    template_dir = Path("templates")
    if file is not None:
        out_file = Path(f".vault/{file}")
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-o", "--output-file", help="Path of output file.")
    return parser
...
if __name__ == "__main__":
    ...
    render_template(args.jinja_file, args.vars_file, file=args.output_file)
