...
exactly ONE artifact, never both:
  ...
  BLUEPRINT owns -> packages, groups, timezone/NTP, locale/keyboard,
                    firewall, systemd services, kernel append args
  KICKSTART owns -> install source, disk layout (via the %pre script),
                    network (incl. hostname), SELinux mode, root/user accounts

  (hostname is emitted in BOTH from the one 'hostname:' key: baked into the
  blueprint payload, and set on the kickstart network line - the latter is what
  actually sticks, since anaconda writes /etc/hostname at install time.)
...
def ks_network(cfg):
    ...
    # anaconda governs the INSTALLED hostname, so it must be set here - otherwise
    # the machine comes up as localhost even when the blueprint baked it into the
    # payload's /etc/hostname. Same 'hostname:' key feeds the blueprint too.
    if cfg.get("hostname"):
        parts.append("--hostname=%s" % cfg["hostname"])
    return [" ".join(parts)]
...
def build_kickstart(cfg, pre_text, warnings, oscap=None):
    ...
    ks = [
        ...
        "#",
        "# Anti-conflict split: the BLUEPRINT owns packages, timezone, locale,",
        "# firewall, services, and kernel args. This KICKSTART owns the install",
        "# source, disk layout (%pre), network (incl. hostname), SELinux, accounts.",
